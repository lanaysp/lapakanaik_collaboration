...
@if($get == 'saved')
    <table class="messenger-list-item m-li-divider @if('user_'.Auth::user()->id == $id && $id != "0") m-list-active @endif">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ 'user_'.Auth::user()->id }}">Pesan Disimpan <span>You</span></p>
...
<table class="messenger-list-item @if($user->id == $id && $id != "0") m-list-active @endif" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <p data-id="{{ $type.'_'.$user->id }}">
            {{ strlen($user->nama_majelis) > 50 ? trim(substr($user->nama_majelis,0,50)).'..' : $user->nama_majelis }}
...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <td>
        <p data-id="{{ $type.'_'.$user->id }}">
            {{ strlen($user->nama_majelis) > 50 ? trim(substr($user->nama_majelis,0,12)).'..' : $user->nama_majelis }}
        </td>
